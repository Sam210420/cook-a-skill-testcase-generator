...
| Mục | Nội dung |
|-----|----------|
| **Tên Skill** | Smart Test Case Generator Pro v2.0 |
| **Việc gì được automate** | Tự động generate test suite hoàn chỉnh từ file spec `.md` — bao gồm test cases (Happy/Negative/Edge/Security), Gherkin scenarios, Traceability mapping, và Coverage Matrix |
| **TRƯỚC: làm tay** | Đọc spec thủ công → tự nghĩ từng test case → dễ miss edge case → không có traceability → mất **~4 tiếng (nửa ngày)** cho 1 feature |
| **SAU: có skill** | Paste spec vào AI → nhận full test suite có structure, Gherkin, Coverage Matrix → chỉ mất **~1 tiếng** (review + adjust) |
| **Tool/AI đã dùng** | Claude (claude.ai) + ChatGPT |
| **Limitation** | Không execute test thật — output cần human review với logic nghiệp vụ phức tạp. Input tối đa 2MB / 10,000 dòng. Security cases là gợi ý, không thay thế pen-testing |
| **Roadmap mở rộng** | Export sang TestRail / Jira / Excel. Auto-detect regression impact khi spec thay đổi. Tích hợp CI/CD để auto-generate test cases khi có PR mới |
